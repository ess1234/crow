<?php
	...
	$strQuery = "select * from ( "
				."	select CRAWLING_SEQ, DOMAIN_SEQ, PARENT_SEQ, URL, " 
				."		(SELECT COUNT(*) cnt FROM url_dir a WHERE a.PARENT_SEQ = b.CRAWLING_SEQ AND a.DOMAIN_SEQ = b.DOMAIN_SEQ GROUP BY PARENT_SEQ) LINK_CNT, " 
				."			REG_DTS  "
				."			from "
				.$table." b "
				.$where
	...
	echo "id,parent,url,link,count,desc"."\n";
	
	while($row=mysql_fetch_array($result)){
		echo $row["CRAWLING_SEQ"].",".$row["PARENT_SEQ"].",".str_replace("http://", "", $row["URL"]).",".$row["URL"].",".$row["LINK_CNT"].","."desc"."\r\n";
	}
