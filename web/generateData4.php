<?php
	...
	$strQuery = " select 	NAV_SEQ, DOMAIN_SEQ, PARENT_SEQ, URL, TARGET_URL, LINK_CNT, ifnull(STATUS, 'none') STATUS, PATH, REPLACE(REPLACE(REPLACE(REPLACE(TITLE, '\r', ''), '\n', ''),'\t',''), ',', '&#44;') TITLE, REG_DTS  "
						." from ".$table
						.$where;
	...
	while($row=mysql_fetch_array($result)){
		...
		if($paths[count($paths)-1] == " nav") $title = $row["TITLE"]."   (".$row["PATH"].")";
	
		echo $row["NAV_SEQ"].",".$row["PARENT_SEQ"].",".$title.",".$row["URL"].",".$linkCnt.",".$row["STATUS"].","."desc"."\r\n";
	}
